...
/*function firstHash(username, password) {

    //salt with username
    let saltedString = username + password;
...
function secondHash(salt, password) {
    ...
    let saltedString = email + password;
    var shasum = crypto.createHash('sha1');
    shasum.update(saltedString);
...
app.post("/login", async (req,res)=>{
    ...
    //query to find the salt value
    //var salt = "";
    // QUERY = "SELECT salt FROM accounts WHERE user_acc=(SELECT uid FROM users WHERE alias=req.body.alias or email=req.body.alias)";
    //secondHash(salt, req.body.pword);
    ...
    let q = sqlstring.format("SELECT (?=??) as auth, ?? FROM ?? WHERE ?? in (SELECT ?? FROM ?? WHERE ??=? or ??=?)",[req.body.pword,"user_pass","user_acc","accounts","user_acc","uid","users","alias",req.body.alias,"email",req.body.alias]);
                    ...
                    conn.query(
                        ...
                        function(err, result, fields){
                            if(err) throw err;
                                ...
                                conn.query(sqlstring.format("UPDATE ?? SET ??=? WHERE ?? = (SELECT ?? FROM ?? WHERE ??=? limit 1) limit 1",["accounts","user_pass",req.body.pword,"user_acc","uid","users","email",req.body.email]), (err, result,fields)=>{
                                    if(err) throw err;
                                    else{
