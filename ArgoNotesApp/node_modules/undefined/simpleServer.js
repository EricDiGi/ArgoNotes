...
app.post("/login", async (req,res)=>{
    ...
    let salt = "";
    let getSaltQuery = sqlstring.format("SELECT ??,?? FROM ?? WHERE ??=(SELECT ?? FROM ?? WHERE ??=? or ??=?)",["user_pass","user_salt","accounts","user_acc","uid","users","alias",req.body.alias,"email",req.body.alias]);
    ...
    let fullyHashedPassword = secondHash(salt, req.body.pword);
                    ...
                    conn.query(
                        ...
                        function(err, result, fields){
                            if(err) throw err;
                                ...
                                conn.query(sqlstring.format("UPDATE ?? SET ??=?,??=? WHERE ?? = (SELECT ?? FROM ?? WHERE ??=? limit 1) limit 1",["accounts","user_pass",req.body.pword,"user_salt",salt,"user_acc","uid","users","email",req.body.email]), (err, result,fields)=>{
                                    if(err) throw err;
                                    else{
